...
class DataProcessing:
    ...
    def process_data(self, raw_data, append: bool = True):
        ...
        if append:
            self.processed_data = self.processed_data._append(
                feature_data, ignore_index=True
            )
        return feature_data
    ...
    def __cleaning_data_row(self, data):
        ...
        try:
            if not self.get_data().empty:
                data = pd.DataFrame(
                    [data.reindex(columns=self.get_data().columns).iloc[0]]
                )
                ref = self.get_data()["Time"][0]
                data = data[self.get_data().columns]
                data["ReferenceTime"] = data.apply(
    ...
    def __feature_engineering(self, clean_data):
        ...
        feature_data = self.__rolling_mean(
            feature_data, "DieselData.Fuelconsumption", window_size
        )
        feature_data = self.__rolling_std(feature_data, "DieselData.Speed", window_size)
        feature_data = self.__change_in_sensor_readings(
            feature_data, "DieselData.Fuelconsumption"
        )
        feature_data = self.__fuel_efficiency(
            feature_data, "DieselData.Fuelconsumption", "DieselData.OperatHours"
        )
        feature_data = self.__cumulative_operating_hours(
            feature_data, "DieselData.OperatHours"
        )
        feature_data = self.__state_transitions(
            feature_data, "MaschineStatus.WorkHydraulikAktiv"
        )
        feature_data = self.__pressure_difference(
