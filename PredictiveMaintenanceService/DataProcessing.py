...
class DataProcessing(Observer):
    def __init__(self):
        ...
        # Battery
        scale_voltage = partial(
            scale_value, column="Bat_Volt", min_value=0, max_value=10
        )
    ...
    def update_subfunction(self, data_type, subfunction_name, new_subfunction):
        ...
        if data_type in self.pipelines:
            subfunctions = [
                subfunc
                for subfunc in self.pipelines[data_type]
                if subfunc.__name__ == subfunction_name
            ]
            if subfunctions:
                ...
                self.logger.error(
                    f"Subfunction {subfunction_name} not found in {data_type} pipeline."
                )
        else:
...
def calculate_derivative(df, column, previous_value, time_elapsed):
    ...
    df[f"derivative_{column}"] = df[column].apply(
        lambda x: (x - previous_value) / time_elapsed
    )
    return df
...
def clip_value(df, column, min_value, max_value):
    ...
    df[f"clip_value{column}"] = df[column].apply(
        lambda x: max(min_value, min(max_value, x))
    )
    return df
...
def scale_value(df, column, min_value, max_value):
    ...
    if isinstance(df, pd.Series):
        ...
        df[f"scale_value_{column}"] = df[column].apply(
...
def log_transform(df, column):
    ...
    df[f"log_transform_{column}"] = df[column].apply(
        lambda x: np.log(x + 1)
